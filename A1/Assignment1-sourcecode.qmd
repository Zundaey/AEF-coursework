---
title: "Advanced Empirical Finance — Assignment 1"
format: html
jupyter: python3
---
...
Import libraries and set the random seed for reproducibility. The seed is fixed at the top of the script as required by Exercise 5.
...
Download daily S&P 500 prices and 13-week T-Bill yields, resample to monthly frequency, and compute monthly excess returns. Annualise the mean and variance by multiplying by 12.
...
Draw betas and idiosyncratic volatilities from uniform distributions to construct 50 synthetic assets. Their true expected returns and covariance matrix follow from the single-factor structure.
...
Compute the true efficient frontier and tangency portfolio analytically using the closed-form solution. This avoids explicit matrix inversion for numerical stability.
...
Plot the mean-variance efficient frontier and mark the tangency portfolio. All values are annualised for readability.
...
For each sample size T, simulate 100 samples from the true DGP and overlay the resulting plug-in frontiers against the true frontier. As N/T increases, estimation error worsens and plug-in frontiers increasingly overstate attainable Sharpe ratios.

```{python}
T_values     = [60, 120, 240, 600]   # sample sizes to compare (months)
...
For each T, simulate 500 samples, compute the plug-in tangency weights, and evaluate their Sharpe ratio using the TRUE parameters. This quantifies how much estimation error erodes out-of-sample performance.
...
Compare five strategies across sample sizes by their median out-of-sample Sharpe ratio, evaluated against the true parameters. Strategies that avoid estimating expected returns (GMV, volatility parity) are expected to be more robust at small T.
